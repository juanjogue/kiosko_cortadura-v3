...
(function () {
    ...
    function applyURLConfig(c) {
        ...
        v = q('duroverride');     if (v) c.guardiaOverrideDuration = Number(v);
    ...
    /* Reconoce URLs válidas como fuente de datos: Google Sheets (pub/edit),
       CSV genérico, enlaces de Drive e imágenes directas. Evita que una URL
       de página web cualquiera se descargue y se "parsee" como CSV basura. */
    function esURLFuente(u) {
        if (!u) return false;
        if (!/^(https?:)?\/\//i.test(u)) return false;
        return u.indexOf('docs.google.com') !== -1 ||
            u.indexOf('drive.google.com') !== -1 ||
            u.indexOf('spreadsheets') !== -1 ||
            /\.csv(\?|$)/i.test(u) ||
    ...
    function loadData() {
        ...
        function loadGaleria() {
            ...
            if (u.indexOf('drive.google.com/file') !== -1 || u.indexOf('drive.google.com/open') !== -1 || /\.(jpg|jpeg|png|webp|gif)/i.test(u)) {
                ...
                anyOk = true; done();
            } else if (esURLFuente(u)) {
                loadText(co(u), function (txt) { onGaleria(csvToObjects(txt)); }, done);
            } else {
                console.warn('URL de galería no reconocida, se omite: ' + u);
                done();
            }
        }
        ...
        function loadCsv(url, onok) {
            if (!url) { done(); return; }
            if (!esURLFuente(url)) {
                console.warn('URL de datos no reconocida, se omite: ' + url);
                done(); return;
            }
            pending++;
...
function saveConfigFromForm() {
        function fromInput(id) { return DOC.getElementById(id).value.trim(); }
        function urlValida(u) { return /^https?:\/\/\S+$/i.test(u); }

        // 1) Validar URLs antes de guardar (mismo criterio que la app pesada).
        //    Un campo vacío NO se valida: se interpreta como "volver al default".
        var checks = [
            ['cfgUrlGuardias', 'URL de Guardias'],
            ['cfgUrlAusencias', 'URL de Ausencias'],
            ['cfgUrlActividades', 'URL de Actividades'],
            ['cfgUrlAlertas', 'URL de Alertas'],
            ['cfgUrlGaleria', 'URL de Galería'],
            ['cfgUrlRss', 'URL del RSS']
        ];
        var i;
        for (i = 0; i < checks.length; i++) {
            var u = fromInput(checks[i][0]);
            if (u !== '' && !urlValida(u)) {
                alert('La "' + checks[i][1] + '" no es válida.\nDebe empezar por http:// o https://');
                return;
            }
        }
        // Coordenadas numéricas si el tiempo está activo
        if (DOC.getElementById('cfgWeatherEnabled').checked) {
            var lat = fromInput('cfgWeatherLat'), lon = fromInput('cfgWeatherLon');
        ...
        // 2) Guardar. Un campo vacío vuelve al valor por defecto: así se puede
        //    "resetear" una URL corrupta sin borrar toda la configuración.
        cfg.urlGuardias = fromInput('cfgUrlGuardias') || DEFAULT_CONFIG.urlGuardias;
        cfg.urlAusencias = fromInput('cfgUrlAusencias') || DEFAULT_CONFIG.urlAusencias;
        cfg.urlActividades = fromInput('cfgUrlActividades') || DEFAULT_CONFIG.urlActividades;
        cfg.urlAlertas = fromInput('cfgUrlAlertas') || DEFAULT_CONFIG.urlAlertas;
        cfg.urlGaleria = fromInput('cfgUrlGaleria') || DEFAULT_CONFIG.urlGaleria;
        cfg.rssEnabled = DOC.getElementById('cfgRssEnabled').checked;
        cfg.urlRss = fromInput('cfgUrlRss') || DEFAULT_CONFIG.urlRss;
        cfg.tituloRss = fromInput('cfgTituloRss') || DEFAULT_CONFIG.tituloRss;
        cfg.weatherEnabled = DOC.getElementById('cfgWeatherEnabled').checked;
        cfg.weatherLat = fromInput('cfgWeatherLat') || DEFAULT_CONFIG.weatherLat;
        cfg.weatherLon = fromInput('cfgWeatherLon') || DEFAULT_CONFIG.weatherLon;
        cfg.weatherCity = fromInput('cfgWeatherCity') || DEFAULT_CONFIG.weatherCity;
        ...
        cfg.galeriaIntervalo = (Number(fromInput('cfgGaleriaIntervalo')) || DEFAULT_CONFIG.galeriaIntervalo / 1000) * 1000;
        cfg.rotacionIntervalo = (Number(fromInput('cfgRotacion')) || DEFAULT_CONFIG.rotacionIntervalo / 1000) * 1000;
        cfg.actualizacionDatos = (Number(fromInput('cfgActualizacion')) || DEFAULT_CONFIG.actualizacionDatos / 1000) * 1000;
